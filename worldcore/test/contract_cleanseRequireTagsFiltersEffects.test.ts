// worldcore/test/contract_cleanseRequireTagsFiltersEffects.test.ts
...
test("[contract] cleanse requireTags filters which matching effects are eligible", async () => {
  ...
  try {
    ...
    // Two debuffs: poison + curse.
    applyStatusEffect(char, {
      id: "se_poison",
      sourceKind: "spell",
      sourceId: "spell_poison",
      name: "Poison",
      durationMs: 10_000,
      maxStacks: 1,
      initialStacks: 1,
    ...
    applyStatusEffect(char, {
      id: "se_curse",
      sourceKind: "spell",
      sourceId: "spell_curse",
      name: "Curse",
      durationMs: 10_000,
      maxStacks: 1,
      initialStacks: 1,
      modifiers: { damageTakenPct: 0.3 },
      tags: ["debuff", "curse"],
    });
    ...
    const spell: SpellDefinition = {
      id: "test_cleanse_self_require_poison",
      name: "Test Cleanse",
      kind: "cleanse_self",
      description: "test",
      classId: "cleric",
      ...
      resourceCost: 0,
      cooldownMs: 0,
      cleanse: { tags: ["debuff"], maxToRemove: 10, requireTags: ["poison"] },
    };
    ...
    // Only poison removed; curse remains.
    assert.equal(after.damageTakenPct, 0.3);
  } finally {
    Date.now = realNow;
  }
});
