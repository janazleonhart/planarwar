// worldcore/test/contract_talkCommand_quests_board_active_filter.test.ts
//
// Contract:
// - `talk <npc> quests active` should show only active quests.
...
function makeChar(): any {
  return {
    id: "char_test_talk_board_active_1",
    userId: "user_test_talk_board_active_1",
    shardId: "prime_shard",
...
test("[contract] talk quests active shows only active quests", async () => {
  ...
  try {
    ...
    const before = await handleTalkCommand(ctx as any, char as any, {
      cmd: "talk",
      args: ["trainer.1", "quests", "active"],
      parts: ["talk", "trainer.1", "quests", "active"],
    });

    assert.match(String(before), /No active quests\./, String(before));
    ...
    assert.match(s, /Active quests:\s+1/i, s);
    assert.match(s, /\[A\].*Rat Culling/i, s);
