// worldcore/test/contract_talkCommand_quests_board_show_tip.test.ts
//
// Contract:
// - talk <npc> quests shows the Town Quest Board
// - output should include a tip that details can be viewed via talk <npc> show <#|id|name>
...
function makeChar(): any {
  return {
    id: "char_test_talk_board_show_tip_1",
    userId: "user_test_1",
    shardId: "prime_shard",
...
test("[contract] talk quests includes tip to show quest details", async () => {
  ...
  try {
    ...
    assert.match(s, /Tip: show details via 'talk\s+(?:\d+|[a-z_]+\.\d+)\s+show\s+<#\|id\|name>'/i, s);
  } finally {
