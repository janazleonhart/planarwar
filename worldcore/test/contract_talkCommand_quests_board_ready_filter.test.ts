// worldcore/test/contract_talkCommand_quests_board_ready_filter.test.ts
//
// Contract:
// - `talk <npc> quests ready` should show only quests that are ready to turn in (completed).
...
function makeChar(): any {
  return {
    id: "char_test_talk_board_ready_1",
    userId: "user_test_talk_board_ready_1",
    shardId: "prime_shard",
...
test("[contract] talk quests ready shows only ready quests", async () => {
  ...
  try {
    ...
    const before = await handleTalkCommand(ctx as any, char as any, {
      cmd: "talk",
      args: ["trainer.1", "quests", "ready"],
      parts: ["talk", "trainer.1", "quests", "ready"],
    });

    assert.match(String(before), /No ready quests\./, String(before));
    ...
    assert.match(s, /Ready quests:\s+1/i, s);
    assert.match(s, /\[C\].*Rat Culling/i, s);
