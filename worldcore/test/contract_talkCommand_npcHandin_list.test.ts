// worldcore/test/contract_talkCommand_npcHandin_list.test.ts
//
// Contract:
//  - talk <npc> handin list should show eligible NPC-policy turn-ins without turning anything in.
...
function makeChar(): CharacterState {
  return {
    id: "char_test_talk_handin_list_1",
    userId: "user_test_1",
    shardId: "prime_shard",
...
test("[contract] talk: handin list shows eligible quests but does not turn them in", async () => {
  const roomId = "prime_shard:0,0";
  const char = makeChar();
  const ctx = makeCtx(char, roomId);

  const q1: QuestDefinition = {
    id: "policy_npc_test_talk_handin_list_1",
    name: "Policy NPC List Test",
    description: "Eligible quest for list.",
    turninPolicy: "npc",
