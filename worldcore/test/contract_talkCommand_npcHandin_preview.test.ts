// worldcore/test/contract_talkCommand_npcHandin_preview.test.ts
//
// Contract:
//  - talk <npc> handin preview should show the same quest preview output as `quest turnin preview`
//    for an eligible NPC-policy quest, and must NOT turn it in.
...
function makeChar(): CharacterState {
  return {
    id: "char_test_talk_handin_preview_1",
    userId: "user_test_1",
    shardId: "prime_shard",
...
test("[contract] talk: handin preview shows quest preview and does not turn it in", async () => {
  const roomId = "prime_shard:0,0";
  const char = makeChar();
  const ctx = makeCtx(char, roomId);

  const q1: QuestDefinition = {
    id: "policy_npc_test_talk_handin_preview_1",
    name: "Policy NPC Preview Test",
    description: "Eligible quest for preview.",
    turninPolicy: "npc",
  ...
  const msg = await handleTalkCommand(ctx as any, char as any, {
    cmd: "talk",
    args: ["1", "handin", "preview"],
    parts: ["talk", "1", "handin", "preview"],
  });

  const s = String(msg);
  assert.match(s, /\[quest\] Preview:/i);
  assert.match(s, /Policy NPC Preview Test/i);
  assert.ok(ensureQuestState(char)[q1.id]?.state === "completed", s);
});
