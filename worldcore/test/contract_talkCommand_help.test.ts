// worldcore/test/contract_talkCommand_help.test.ts
//
// Contract:
// - `talk <npc> help` should render a compact list of talk subcommands.
...
test("[contract] talk help lists talk-driven quest + handin shortcuts", async () => {
  ...
  const out = await handleTalkCommand(ctx as any, char as any, {
    cmd: "talk",
    args: ["trainer.1", "help"],
    parts: ["talk", "trainer.1", "help"],
  });

  const s = String(out);
  assert.match(s, /\[talk\]\s+You speak with Town Trainer\./i, s);
  assert.match(s, /\[talk\]\s+Commands:/i, s);
  assert.match(s, /talk\s+trainer\.1\s+quests/i, s);
