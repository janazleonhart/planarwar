// worldcore/test/contract_spellCleanseSingleAlly_denyPath_noCostNoCooldown.test.ts
//
// Contract: cleanse (single ally) must resolve + deny invalid targets BEFORE
...
class FakeSessions {
  ...
  constructor(sessions: any[]) { this.sessions = sessions; }
  getAllSessions() { return this.sessions; }
}

class FakeEntities {
  private byOwner = new Map<string, any>();
  set(ownerSessionId: string, ent: any) { this.byOwner.set(ownerSessionId, ent); }
  getEntityByOwner(ownerSessionId: string) { return this.byOwner.get(ownerSessionId) ?? null; }
}

test("[contract] cleanse_single_ally deny-path: invalid target does not spend mana or start cooldown", async () => {
  const caster = makeChar("c1", "Caster");

  const spell: SpellDefinition = {
    id: "contract_cleanse_single_ally",
    ...
    resourceCost: 10,
    cooldownMs: 5_000,
    cleanse: { tags: ["dot", "debuff"], maxToRemove: 5 },
  } as any;
