// worldcore/test/contract_talkCommand_quests_board_turned_filter.test.ts
//
// Contract:
// - `talk <npc> quests turned|done` should show only quests that have been turned in.
...
function makeChar(): any {
  return {
    id: "char_test_talk_board_turned_1",
    userId: "user_test_talk_board_turned_1",
    shardId: "prime_shard",
...
test("[contract] talk quests turned shows only turned-in quests", async () => {
  ...
  try {
    ...
    const before = await handleTalkCommand(ctx as any, char as any, {
      cmd: "talk",
      args: ["trainer.1", "quests", "turned"],
      parts: ["talk", "trainer.1", "quests", "turned"],
    });

    assert.match(String(before), /No turned-in quests\./, String(before));
    ...
    assert.match(s, /Turned-in quests:\s+1/i, s);
    assert.match(s, /\[T\].*Rat Culling/i, s);
    assert.ok(!/Rat Tail Collection/i.test(s), s);
  } finally {
