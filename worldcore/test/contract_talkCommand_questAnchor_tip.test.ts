// worldcore/test/contract_talkCommand_questAnchor_tip.test.ts
//
// Contract:
// - When talking to a quest-anchor NPC (trainer/questgiver), the default `talk <npc>`
//   output should include a lightweight tip that advertises the talk-driven quest
//   board path.
...
function makeChar(): any {
  return {
    id: "char_test_talk_anchor_tip_1",
    userId: "user_test_1",
    shardId: "prime_shard",
    name: "Testy",
    classId: "warrior",
...
test("[contract] talk to quest-anchor NPC includes quest board discoverability tip", async () => {
  ...
  const out = await handleTalkCommand(ctx as any, char as any, {
    cmd: "talk",
    args: ["trainer.1"],
    parts: ["talk", "trainer.1"],
  });

  const s = String(out);
  assert.ok(s.includes("You speak with Town Trainer"), s);
  assert.match(s, /Tip:\s+view quests via 'talk\s+trainer\.1\s+quests'/i, s);
});
