// worldcore/test/contract_talkCommand_npcAutoHandin_singleEligible.test.ts
//
// Contract: talk <npc> handin (or turnin/complete) should immediately turn in the quest
// when exactly one NPC-policy quest is eligible for that NPC.
...
function makeChar(): CharacterState {
  return {
    id: "char_test_talk_autohandin_1",
    userId: "user_test_1",
    shardId: "prime_shard",
...
test("[contract] talk: 'handin' auto-turns-in when exactly one NPC-policy quest is eligible", async () => {
  const roomId = "prime_shard:0,0";
  const char = makeChar();
  const ctx = makeCtx(char, roomId);

  const quest: QuestDefinition = {
    id: "policy_npc_test_talk_autohandin",
    name: "Policy NPC Test (Talk AutoHandin)",
    description: "Talking with 'handin' should immediately turn in when only one is eligible.",
    turninPolicy: "npc",
  ...
  assert.ok(s.includes("You turn in"), s);

  // The quest should now be marked turned_in.
  assert.equal(ensureQuestState(char)[quest.id]?.state, "turned_in");
});
