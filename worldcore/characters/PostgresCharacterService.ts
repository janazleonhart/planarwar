...
export type CharacterPatch = Partial<Pick<
  CharacterState,
  | "posX" | "posY" | "posZ"
  | "lastRegionId" | "appearanceTag"
  | "attributes" | "inventory" | "equipment"
  | "spellbook" | "abilities" | "progression"
>>;
...
export class PostgresCharacterService {
  ...
  async loadCharacterForUser(userId: string, charId: string): Promise<CharacterState | null> {
    const r = await db.query<CharacterRow>(
    ...
    const result = await db.query<CharacterRow>(
      `
        INSERT INTO characters (
          user_id, shard_id, name, class_id,
          level, xp,
          pos_x, pos_y, pos_z,
          last_region_id, appearance_tag,
          attributes, inventory, equipment, spellbook, abilities, progression,
          state_version
        )
        VALUES (
          $1, $2, $3, $4,
  ...
  async patchCharacter(userId: string, charId: string, patch: CharacterPatch): Promise<CharacterState | null> {
    ...
    const merged: CharacterState = {
      ...
      inventory: patch.inventory ? { ...existing.inventory, ...patch.inventory } as any : existing.inventory,
      equipment: patch.equipment ? { ...existing.equipment, ...patch.equipment } : existing.equipment,
      spellbook: patch.spellbook ? { ...existing.spellbook, ...patch.spellbook } as any : existing.spellbook,
      abilities: patch.abilities ? { ...existing.abilities, ...patch.abilities } : existing.abilities,
      progression: patch.progression ? { ...existing.progression, ...patch.progression } as any : existing.progression,
    };
  ...
  async grantXp(userId: string, charId: string, deltaXp: number): Promise<CharacterState | null> {
    const existing = await this.loadCharacterForUser(userId, charId);
    if (!existing) return null;
  ...
  private getPerLevelGains(classId: string): Attributes {
    return getPerLevelAttributesForClass(classId);
  }

  async learnSpell(userId: string, charId: string, spellId: string, rank = 1): Promise<CharacterState | null> {
    const existing = await this.loadCharacterForUser(userId, charId);
    if (!existing) return null;
  ...
  async equipItem(userId: string, charId: string, slot: string, item: any): Promise<CharacterState | null> {
    const existing = await this.loadCharacterForUser(userId, charId);
    if (!existing) return null;
